...
export class EditProgramComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.svc.getProgram(this.id).subscribe({
      next: (p) => {
  ...
  submit(): void {
    ...
    this.svc.updateProgram(this.id, this.form.value as any).subscribe({
      next: () => {
        this.router.navigate(['/program-manager/program-management']);
      },
      error: (e) => {
        this.loading.set(false);
        this.error.set(e?.error?.message ?? 'An error occurred while updating the program.');
      }
    });
  }
}